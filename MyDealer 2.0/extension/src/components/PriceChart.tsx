...
const PriceChart: React.FC<PriceChartProps> = ({ productId, amazonUrl, apiUrl = 'http://localhost:8000' }) => {
  ...
  useEffect(() => {
    const fetchPriceHistory = async () => {
      try {
        ...
        // API key is optional - backend will skip auth if not set
        const headers: HeadersInit = {
          'Content-Type': 'application/json',
        };
        
        if(amazonUrl) {
          // POST endpoint - automatically extracts domain and currency
  ...
  const handleTrackPrice = () => {
    // Demo alert - TODO: Implement actual tracking
  ...
  // Prepare chart data
  const chartData = hist.history.map((point) => ({
      ...
      <div className="price-stats">
        <div className="stat-item">
          ...
          <span className="stat-value current">
            {formatPrice(hist.currentPrice, hist.currency)}
          </span>
        </div>
        {hist.lowestPrice && (
          <div className="stat-item">
            ...
            <span className="stat-value lowest">
              {formatPrice(hist.lowestPrice, hist.currency)}
            </span>
          </div>
        )}
        {hist.highestPrice && (
          <div className="stat-item">
            ...
            <span className="stat-value highest">
              {formatPrice(hist.highestPrice, hist.currency)}
            </span>
          </div>
        )}
        {hist.averagePrice && (
          <div className="stat-item">
            ...
            <span className="stat-value average">
              {formatPrice(hist.averagePrice, hist.currency)}
            </span>
          </div>
        )}
      </div>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
            ...
            <YAxis 
              stroke="#666"
              tick={{ fontSize: 12 }}
              tickFormatter={(value) => formatPrice(value, hist.currency)}
            />
            <Tooltip
              formatter={(value: number) => formatPrice(value, hist.currency)}
